...
def job_config = [
        job: [
                name: "mikolajkowy_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "feature-1670489553328"
        ]
]
...
def customLib = library identifier: 'mikolajkowy_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/mikolajkowy_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.mikolajkowy.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "mikolajkowy", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("mikolajkowy_Pipeline_develop");

job.with {

    authenticationToken('q7b28j31uwceXNPnueUoSqkGGToy2p07')
